 ...
 * <code>JmxDatacollectionConfig</code> and provides some methods to make life a
 * little easier. So in the future we may support multiple data soruces (and
 * therefore multiple JmxDatacollectionConfigs), and so on. Or we find out, that
 * we do not need this class, then we will remove it.
 *
 * @author m.v.rueden
 */
public class InternalModel {
    ...
    /**
     * Checks if the given
     * <code>rawModel</code> is not null and does have Mbeans (count can be 0,
     * but not NULL).
     *
     * @param rawModel
     ...
     * this, we simply clone the original
     * <code>JmxDatacollectionConfig</code> loaded at the beginning. After that
     * we remove all MBeans/Attribs/CompMembers/CompAttribs and add them
     * manually with the changes made in the gui.
     *
     * @param controller the MBeansController of the MbeansView (is needed to
     * determine the changes made in gui)
     * @return
     */
    ...
    public JmxDatacollectionConfig getRawModelIncludeSelection(MBeansController controller) {
        ...
        for (Mbean mbean : selectedMbeans) {
            ...
            if (!exportBean.getAttrib().isEmpty()) {
                exportBeans.add(exportBean); //no attributes selected, don't add bean
            }
